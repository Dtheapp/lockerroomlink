...
const AdminLayout: React.FC = () => {
  ...
  const activeClasses = 'bg-orange-600 text-white shadow-[0_0_15px_rgba(234,88,12,0.5)]';
  const inactiveClasses = 'text-slate-700 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-zinc-950 hover:text-orange-600 dark:hover:text-orange-400';
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-black overflow-hidden">
      ...
      <aside
        ...
        } flex flex-col bg-slate-50 dark:bg-zinc-950 border-r border-slate-200 dark:border-zinc-800 p-4 h-full overflow-hidden ${
          isMobile 
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="md:hidden text-slate-600 dark:text-slate-400 hover:text-orange-600 dark:hover:text-orange-400 transition-colors flex-shrink-0 ml-2"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <nav className="flex-1 space-y-1">
          {adminNavItems.map((item) => (
            ...
            >
              ...
              <span className={`transition-opacity duration-300 ${sidebarOpen ? 'opacity-100 ml-3' : 'opacity-0 w-0'}`}>{item.name}</span>
            </NavLink>
          ))}
        </nav>

        <div className="border-t border-slate-200 dark:border-zinc-800 pt-4 mt-auto space-y-2">
          <button
            onClick={toggleTheme}
          ...
          >
            ...
            <span className={`transition-opacity duration-300 ${sidebarOpen ? 'opacity-100 ml-3' : 'opacity-0 w-0'}`}>
              Light Mode
            </span>
          </button>
          <button
          ...
          >
            ...
            <span className={`transition-opacity duration-300 ${sidebarOpen ? 'opacity-100 ml-3' : 'opacity-0 w-0'}`}>Sign Out</span>
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Mobile Header */}
        <div className="md:hidden flex items-center justify-between bg-slate-50 dark:bg-zinc-950 border-b border-slate-200 dark:border-zinc-800 p-4">
          <button
        ...
        <div className="hidden md:flex items-center bg-slate-50 dark:bg-zinc-950 border-b border-slate-200 dark:border-zinc-800 p-4">
          <button
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="text-slate-600 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 hover:bg-slate-200 dark:hover:bg-zinc-900 transition-all rounded-lg p-2"
