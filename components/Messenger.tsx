...
const Messenger: React.FC = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] gap-6">
      ...
      <div className={`w-full md:w-1/3 flex flex-col bg-slate-900 rounded-xl border border-slate-800 overflow-hidden ${activeChat ? 'hidden md:flex' : 'flex'}`}>
          <div className="p-4 border-b border-slate-800 flex justify-between items-center">
              <h2 className="text-xl font-bold text-white">Messenger</h2>
              <button onClick={() => setIsSearchOpen(!isSearchOpen)} className="bg-sky-600 hover:bg-sky-700 text-white p-2 rounded-lg">
          ...
          {/* SEARCH UI */}
          {isSearchOpen && (
              <div className="p-4 bg-slate-800/50 border-b border-slate-800 animate-in slide-in-from-top-2">
                  <form onSubmit={handleSearchUser} className="flex gap-2 mb-2">
                      <input 
                        value={searchQuery}
                        onChange={e => setSearchQuery(e.target.value)}
                        placeholder="Search Name or Username..." 
                        className="flex-1 bg-slate-950 border border-slate-700 rounded p-2 text-white text-sm"
                        autoFocus
                      />
                      <button type="submit" className="bg-slate-700 text-white px-3 rounded text-sm">{searching ? '...' : 'Go'}</button>
                  </form>
                  {searchResults.length > 0 && (
                      <div className="space-y-2 max-h-60 overflow-y-auto">
                          {searchResults.map(u => (
                              <div key={u.uid} onClick={() => startChat(u)} className="flex items-center justify-between p-3 hover:bg-slate-700 rounded cursor-pointer bg-slate-900 border border-slate-800">
                                  <div>
                                      <p className="text-white text-sm font-bold">{u.name}</p>
                                      <p className="text-xs text-sky-400">@{u.username}</p>
                                  </div>
                                  <span className="text-[10px] text-slate-400 uppercase bg-slate-800 px-2 py-1 rounded border border-slate-700">{u.role}</span>
                              </div>
                          ))}
                      </div>
                  )}
                  {searchResults.length === 0 && searchQuery && !searching && <p className="text-xs text-slate-500">No user found.</p>}
              </div>
          )}
                  ...
                  chats.map(chat => {
                      ...
                      return (
                        <div 
                            ...
                            className={`p-4 border-b border-slate-800 cursor-pointer hover:bg-slate-800/50 transition-colors ${activeChat?.id === chat.id ? 'bg-slate-800 border-l-4 border-l-sky-500' : ''}`}
                        >
                            <div className="flex justify-between items-start mb-1">
                                <h3 className="font-bold text-white text-sm">{other.username}</h3>
                                <span className="text-[10px] text-slate-500 uppercase bg-slate-800 px-1.5 rounded">{other.role}</span>
                            </div>
      ...
      <div className={`w-full md:w-2/3 flex flex-col bg-slate-900 rounded-xl border border-slate-800 overflow-hidden ${!activeChat ? 'hidden md:flex' : 'flex'}`}>
          {activeChat ? (
              <>
                {/* Chat Header */}
                <div className="p-4 border-b border-slate-800 flex items-center gap-3 bg-slate-800/30">
                    <button onClick={() => setActiveChat(null)} className="md:hidden text-slate-400 hover:text-white mr-2">← Back</button>
                    ...
                    <div>
                        ...
                        <p className="text-xs text-sky-400 uppercase tracking-wider">{getOtherParticipant(activeChat).role}</p>
                    </div>
                </div>
                    ...
                    {messages.map((msg, index) => {
                        ...
                        return (
                            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[75%] rounded-2xl px-4 py-2 text-sm ${isMe ? 'bg-sky-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-200 rounded-bl-none'}`}>
                                    <p>{msg.text}</p>
                                </div>
                            </div>
                        );
                ...
                <form onSubmit={sendMessage} className="p-4 border-t border-slate-800 bg-slate-900 flex gap-2">
                    <input 
                        ...
                        className="flex-1 bg-slate-950 border border-slate-700 rounded-full px-4 py-2 text-white focus:outline-none focus:border-sky-500 transition-colors"
                    />
                    <button type="submit" disabled={!newMessage.trim()} className="bg-sky-600 hover:bg-sky-700 text-white p-2 rounded-full transition-colors disabled:opacity-50">
