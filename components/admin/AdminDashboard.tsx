        ...
        <>
            ...
            {statCards.map(card => (
                <div 
                    ...
                    className="bg-slate-900 p-6 rounded-lg flex items-center gap-6 border border-slate-800 shadow-lg cursor-pointer hover:border-sky-500/50 hover:bg-slate-800 transition-all active:scale-[0.98]"
                >
                    <div className={`p-4 bg-slate-950 rounded-lg border border-slate-800 ${card.color}`}>
            ...
            <div className="bg-slate-900 rounded-lg border border-slate-800 shadow-lg overflow-hidden">
                ...
                <div className="md:hidden">
                    {recentUsers.map((user, index) => (
                        <div key={index} className="p-4 border-b border-slate-800 last:border-0">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wider ${user.role === 'Coach' ? 'bg-purple-900/50 text-purple-400 border border-purple-800' : 'bg-emerald-900/50 text-emerald-400 border border-emerald-800'}`}>
                                    {user.role}
                                </span>
                            </div>
                            ...
                            {recentUsers.map((user, index) => (
                                <tr key={index} className="border-b border-slate-800 hover:bg-slate-800/50">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wider ${user.role === 'Coach' ? 'bg-purple-900/50 text-purple-400' : 'bg-emerald-900/50 text-emerald-400'}`}>
                                            {user.role}
                                        </span>
                                    </td>
                                </tr>
