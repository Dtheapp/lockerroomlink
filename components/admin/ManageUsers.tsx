...
import { Trash2, Link, User, Shield, AtSign } from 'lucide-react';
...
const ManageUsers: React.FC = () => {
  ...
  const renderActions = (user: UserProfile, isMobile = false) => (
    ...
    <div className="space-y-6">
      ...
      <div className="md:hidden space-y-4">
          {loading ? <p className="text-center text-slate-500 dark:text-slate-400">Loading users...</p> : 
           ...
           filteredUsers.map(user => (
              <div key={user.uid} className="bg-slate-50 dark:bg-zinc-950 rounded-xl border border-slate-200 dark:border-zinc-800 p-5 shadow-lg">
                  ...
                  <div className="text-sm text-slate-600 dark:text-slate-400 space-y-2 mb-2">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <span className={user.teamId ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-500 italic'}>
                              Team: {user.teamId ? <span className="font-mono bg-slate-300 dark:bg-zinc-900 px-1 rounded">{user.teamId}</span> : 'Unassigned'}
                          </span>
                      </div>
                  </div>
          ...
          <table className="w-full text-sm text-left text-slate-700 dark:text-slate-300">
            ...
            <tbody>
              {loading ? (
                ...
                filteredUsers.map(user => (
                                <tr key={user.uid} className="bg-slate-50 dark:bg-zinc-950 border-b border-slate-200 dark:border-zinc-800 hover:bg-slate-100 dark:hover:bg-black">
                    <td className="px-6 py-4 font-medium text-slate-900 dark:text-white whitespace-nowrap">
                        {user.name}
                        {user.role === 'Parent' && <span className="block text-xs text-slate-500 font-normal">{user.email}</span>}
                    </td>
                    <td className="px-6 py-4 font-mono text-orange-600 dark:text-orange-400">{user.username || '---'}</td>
                    <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded text-xs font-bold ${user.role === 'Coach' ? 'bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-400' : 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400'}`}>
                            {user.role}
                        </span>
                    </td>
      ...
      {isModalOpen && selectedUser && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-200 dark:bg-zinc-950 p-6 rounded-lg w-full max-w-md border border-slate-200 dark:border-zinc-800">
            ...
            <div className="space-y-4">
               ...
               <select id="team" value={selectedTeamId} onChange={(e) => setSelectedTeamId(e.target.value)} className="w-full bg-white dark:bg-zinc-900 p-2 rounded border border-slate-300 dark:border-zinc-700 text-slate-900 dark:text-white">
            ...
            <div className="flex justify-end gap-4 mt-6">
              <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded bg-slate-400 dark:bg-zinc-700 hover:bg-slate-500 dark:hover:bg-zinc-600 text-white">Cancel</button>
              <button onClick={handleAssignTeam} className="px-4 py-2 rounded bg-orange-600 hover:bg-orange-700 text-white">Assign</button>
            </div>
          </div>
        </div>
      )}
