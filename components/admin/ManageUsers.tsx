...
const ManageUsers: React.FC = () => {
  ...
  const renderActions = (user: UserProfile, isMobile = false) => (
      <div className={`flex items-center gap-2 ${isMobile ? 'mt-4 pt-4 border-t border-slate-800 w-full' : 'justify-end'}`}>
          {!user.teamId && (
            <button 
                onClick={() => openAssignModal(user)} 
                className={`flex items-center justify-center gap-1 rounded-md bg-sky-900/30 text-sky-400 hover:bg-sky-900/50 hover:text-white transition-colors font-medium border border-sky-800/50 ${isMobile ? 'flex-1 py-2 text-sm' : 'px-3 py-1.5 text-xs'}`}
            >
          ...
          <button 
            ...
            className={`flex items-center justify-center gap-1 rounded-md bg-red-900/30 text-red-400 hover:bg-red-900/50 hover:text-white transition-colors font-medium border border-red-800/50 ${isMobile ? 'flex-1 py-2 text-sm' : 'px-3 py-1.5 text-xs'}`}
          >
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* FILTER CONTROLS */}
          <div className="flex flex-wrap bg-slate-900 p-1 rounded-lg border border-slate-700">
      ...
      <div className="md:hidden space-y-4">
          {loading ? <p className="text-center text-slate-500">Loading users...</p> : 
           ...
           filteredUsers.map(user => (
              <div key={user.uid} className="bg-slate-900 rounded-xl border border-slate-800 p-5 shadow-lg">
                  <div className="flex justify-between items-start mb-3">
                      <div>
                          <h3 className="text-lg font-bold text-white flex items-center gap-2">
                              {user.name}
                          </h3>
                          <div className="flex items-center gap-1 text-xs text-sky-400 font-mono mt-1">
                              <AtSign className="w-3 h-3" /> {user.username || 'No Username'}
                          </div>
                      </div>
                      <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wider ${user.role === 'Coach' ? 'bg-purple-900/50 text-purple-400 border border-purple-800' : 'bg-emerald-900/50 text-emerald-400 border border-emerald-800'}`}>
                          {user.role}
                      </span>
                  </div>

                  <div className="text-sm text-slate-400 space-y-2 mb-2">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <span className={user.teamId ? 'text-white' : 'text-slate-500 italic'}>
                              Team: {user.teamId ? <span className="font-mono bg-slate-800 px-1 rounded">{user.teamId}</span> : 'Unassigned'}
                          </span>
                      </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 p-6 rounded-lg w-full max-w-md">
            ...
            <div className="space-y-4">
               ...
               <select id="team" value={selectedTeamId} onChange={(e) => setSelectedTeamId(e.target.value)} className="w-full bg-slate-800 p-2 rounded border border-slate-700 text-white">
            ...
            <div className="flex justify-end gap-4 mt-6">
              <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded bg-slate-700 hover:bg-slate-600 text-white">Cancel</button>
              <button onClick={handleAssignTeam} className="px-4 py-2 rounded bg-sky-600 hover:bg-sky-700 text-white">Assign</button>
            </div>
          </div>
        </div>
      )}
