    ...
    return (
      <div className="p-6 max-w-lg mx-auto">
        <div className="bg-red-900/20 border border-red-500/50 rounded-xl p-6 text-center">
          ...
          {userData.banReason && (
            <p className="text-sm text-zinc-500 italic">Reason: {userData.banReason}</p>
          )}
        ...
        <div className="p-6 -mt-12">
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4">
            ...
            <div className="relative">
              <div className="w-24 h-24 rounded-full border-4 border-zinc-900 bg-zinc-800 overflow-hidden">
                {photoURL ? (
            ...
            <div className="flex-1 text-center sm:text-left">
              {isEditing ? (
                <input
                  ...
                  className="text-xl font-bold bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-1 text-white w-full max-w-xs"
              ...
              <div className="flex items-center justify-center sm:justify-start gap-2 mt-1">
                <span className="px-2 py-0.5 bg-purple-600/20 text-purple-400 text-xs font-medium rounded-full flex items-center gap-1">
                  <Star size={10} /> Fan
                </span>
              </div>
            </div>
            ...
            <div>
              {isEditing ? (
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setIsEditing(false); setDisplayName(userData.name || ''); setBio(userData.bio || ''); }} className="px-4 py-2 bg-zinc-700 text-white rounded-lg hover:bg-zinc-600 transition-colors">
                    <X size={16} />
                  </button>
                </div>
              ) : (
                <button onClick={() => setIsEditing(true)} className="px-4 py-2 bg-zinc-800 text-white rounded-lg hover:bg-zinc-700 transition-colors flex items-center gap-2">
          ...
          <div className="mt-4">
            ...
            {isEditing ? (
              ...
              <p className="text-sm text-zinc-300">{userData.bio || 'No bio yet'}</p>
            )}
          </div>
          ...
          <div className="mt-6 grid grid-cols-3 gap-4">
            <div className="bg-zinc-800/50 rounded-lg p-3 text-center">
              ...
              <div className="text-xl font-bold text-white">{userData.followedAthletes?.length || 0}</div>
              <div className="text-xs text-zinc-400">Following</div>
            </div>
            <div className="bg-zinc-800/50 rounded-lg p-3 text-center">
              <Heart className="w-5 h-5 text-pink-400 mx-auto mb-1" />
              <div className="text-xl font-bold text-white">{totalKudosGiven}</div>
              <div className="text-xs text-zinc-400">Kudos Given</div>
            </div>
            <div className="bg-zinc-800/50 rounded-lg p-3 text-center">
              <Shield className="w-5 h-5 text-green-400 mx-auto mb-1" />
              <div className="text-xl font-bold text-white">Good</div>
              <div className="text-xs text-zinc-400">Standing</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
        ...
        {loadingFollowed ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-xs text-zinc-500 mt-1">Browse public athlete profiles to start following!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {followedAthletes.map((athlete) => (
              <div key={`${athlete.team?.id}_${athlete.player.id}`} className="bg-zinc-800/50 rounded-lg p-4 flex items-center gap-3 group hover:bg-zinc-800 transition-colors">
                {/* Athlete Photo */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {athlete.player.username && (
                    <p className="text-xs text-zinc-400">@{athlete.player.username}</p>
                  )}
                  {athlete.team && (
                    <p className="text-xs text-purple-400 truncate">{athlete.team.name}</p>
                  )}
                </div>
      ...
      {unfollowConfirm && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 rounded-xl border border-zinc-700 p-6 max-w-sm w-full">
            <h3 className="text-lg font-bold text-white mb-2">Unfollow {unfollowConfirm.player.name}?</h3>
