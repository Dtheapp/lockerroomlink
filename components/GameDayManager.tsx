...
import { doc, updateDoc, serverTimestamp, Timestamp, setDoc, collection, query, where, getDocs } from 'firebase/firestore';
...
const GameDayManager: React.FC<GameDayManagerProps> = ({
  ...
  // Mark game as completed and create game record for stats
  const endGame = async () => {
    if (!isCoach || !game.id || !teamId) return;
    setIsUpdating(true);
    try {
      // Update event status
      await updateDoc(doc(db, 'events', game.id), {
      ...
      // Check if game already exists in teams/{teamId}/games (by eventId)
      ...
      if (existingGames.empty) {
        // Create game record in teams/{teamId}/games for stats tracking
        const currentYear = new Date().getFullYear();
        const gameDate = game.date || new Date().toISOString().split('T')[0];
        
        await setDoc(doc(collection(db, 'teams', teamId, 'games')), {
          eventId: game.id,  // Link to original event
          date: gameDate,
          opponent: game.opponent || 'Unknown',
          isHome: game.isHome ?? true,
          teamScore: game.isHome ? localHomeScore : localAwayScore,
          opponentScore: game.isHome ? localAwayScore : localHomeScore,
    ...
    <div className={`rounded-2xl overflow-hidden ${
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {/* Collapsed: Show compact scores inline */}
          {isCollapsed && (gameState === 'live' || gameState === 'completed') ? (
            <div className="flex items-center gap-3">
              {/* Compact Score Display with Animation */}
              <div className="flex items-center gap-2 bg-white/20 rounded-lg px-3 py-1.5">
                ...
                <div className="relative">
                  ...
                  {homeScoreChange !== null && (
                    <span className="absolute -top-4 left-1/2 -translate-x-1/2 text-green-300 font-bold text-sm animate-bounce whitespace-nowrap">
                      +{homeScoreChange}
                    </span>
                  )}
                </div>
          ...
          ) : isCollapsed && gameState === 'pregame' && countdown ? (
            <div className="flex items-center gap-3">
      ...
      <div className="p-6">
        ...
        {/* Live Stream Section - Only show when game is NOT completed */}
        {gameState !== 'completed' && (hasLiveStream || isCoach) && (
          <div className={`rounded-xl overflow-hidden mb-6 ${
            theme === 'dark' ? 'bg-black/30' : 'bg-slate-100'
          }`}>
            {hasLiveStream ? (
              <div>
                {/* Live indicator header with fullscreen option */}
                  ...
                  <div className="flex items-center gap-2">
                    {isCoach && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-medium bg-red-600 hover:bg-red-700 text-white transition"
                      >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-medium bg-white/20 hover:bg-white/30 text-red-600 transition"
                    >
                ...
                {/* Embedded stream - always visible */}
      ...
      {showEndStreamConfirm && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className={`rounded-xl p-6 max-w-md w-full border ${
            theme === 'dark' 
