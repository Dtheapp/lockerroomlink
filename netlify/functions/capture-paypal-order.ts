...
const handler: Handler = async (event) => {
  ...
  try {
    const body: CaptureOrderRequest = JSON.parse(event.body || '{}');
    ...
    const response = await fetch(
      `${PAYPAL_API_BASE}/v2/checkout/orders/${body.paypalOrderId}/capture`,
      {
        ...
        headers: {
          ...
          'PayPal-Request-Id': `capture_${body.paypalOrderId}_${Date.now()}`,
        },
      }
    );
    ...
    if (status !== 'COMPLETED') {
      console.warn('Payment capture not completed:', status);
      return {
        statusCode: 400,
        body: JSON.stringify({
          success: false,
    ...
    // 1. Update the registration in Firestore with payment info
    // 2. Add athlete to roster
    // 3. Send confirmation email
    // 4. Notify coach if enabled
    
    // For now, return success - the frontend will handle Firestore updates
